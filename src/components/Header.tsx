      ...
      <div className="mx-auto flex h-14 items-center justify-between px-4 md:px-6">
        {pathname === '/' ? (
          <div className="min-w-0 leading-tight">
            <h1 className="text-base font-semibold tracking-tight text-foreground">{t('nav.dashboard')}</h1>
            <p className="mt-0.5 text-[11px] text-muted-foreground">{t('dashboard.subtitle')}</p>
          </div>
        ) : <div />}
        <div className="app-no-drag flex items-center gap-3">
          {modules.map((module) => {
            ...
            return (
              <NavLink key={module.id} to={route} title={text(runState.label)} className={`flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs transition-colors ${pathname === route ? 'bg-primary/10 text-primary ring-1 ring-primary/20' : 'text-muted-foreground hover:text-foreground'}`}>
