...
async function verifyNativeBridge(): Promise<void> {
  ...
  const logDirectory = path.join(manifestDirectory, 'logs')
  const service = new VrOverlayService(resources, (_file, args) => { registryCalls.push(args) }, undefined, manifestDirectory, logDirectory)
  try {
    ...
    assert.equal(manifest.api_layer.implementation_version, '4')
    assert.equal(manifest.api_layer.functions?.xrNegotiateLoaderApiLayerInterface, 'xrNegotiateLoaderApiLayerInterface')
    service.cleanupStaleRegistration()
    assert.equal(registryCalls.length, 2)
    assert.equal(registryCalls[0]?.[0], 'delete')
    const enabled = service.setDisplayMode('vr')
    assert.equal(enabled.available, true)
    assert.equal(enabled.bridgeRunning, true)
    assert.equal(registryCalls[2]?.[0], 'add')
    assert.equal(registryCalls[2]?.[1], 'HKCU\\SOFTWARE\\DCSHUB')
    assert.equal(registryCalls[2]?.at(-2), path.resolve(logDirectory))
    assert.match(registryCalls[3]?.[1] || '', /^HKLM\\/)
    assert.equal(registryCalls[3]?.at(-2), '0')
    assert.match(registryCalls[4]?.[1] || '', /^HKCU\\/)
    assert.equal(registryCalls[4]?.at(-2), '1')
    service.beginFrames()
    ...
    assert.equal(registryCalls.at(-1)?.[0], 'delete')
  } finally {
    service.dispose()
