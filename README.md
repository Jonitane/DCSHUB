...
V1.8 起还提供“DCS 智能手册”：用户指定自己的手册目录后，DCSHUB 建立可长期复用的本地索引，并通过用户自行配置的 DeepSeek API 根据手册原文回答问题。
...
- 智能手册：多格式永久索引、DCS 客户端手册复制、Chuck 官方手册按机型下载，以及带来源引用的 DeepSeek 文字问答。
...
- 索引保存在用户数据目录；仅在首次建库、用户手动刷新或导入手册后更新。
- 使用文件元数据和 SHA-256 内容指纹判断变化，文件未变化时不会重新解析。
- 自动识别常见机型和中、英、俄文内容。
- 一键复制 DCS 安装目录中各模块的 `Doc` / `Docs` 手册。
- 从 [Chuck's Guides 官方网站](https://chucksguides.com/)按用户选择下载对应机型 PDF；手册不包含在 DCSHUB 发布包或源码仓库中。
- 用户自行提供 DeepSeek API Key，默认使用 `deepseek-v4-flash`，也可以选择 `deepseek-v4-pro`。
- API Key 使用 Electron/Windows 安全存储加密；不会写入日志或以明文保存。
- 只将问题和本地索引命中的少量文字片段发送给 DeepSeek，回答中保留来源编号并可打开原手册。
- 当前版本仅处理文字。截图提问接口已经预留，待后续接入视觉模型。

不同语言之间的检索会在本地命中不足时调用一次短查询扩展，再从本地永久索引读取原文。DeepSeek 是在线服务，使用费用、可用性和数据处理规则由用户所选账号及服务条款决定。
